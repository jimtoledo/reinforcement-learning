...
class Experience:
    ...
    shared_actions: torch.Tensor
    rewards: torch.Tensor
    shared_next_states: torch.Tensor 
...
class MADDPGAgent():
    ...
    def _init_model(self, env, agent, memory, gamma=1.0, batch_size=None):
        ...
        self.memory = memory
        self.batch_size = batch_size if batch_size else self.memory._batch_size
    ...
    def _optimize_model(self, env, agents, batch_size=None, update_policy=True):
        ...
        states = batch.shared_states[:, self._state_index:self._state_index+self._nS]
        
        i = slice(self._agent_index, self._agent_index+1)
        rewards, is_terminals = batch.rewards[:, i], batch.is_terminals[:, i]
        with torch.no_grad():
            #get actions in next states FOR ALL agents for target value calculation
            shared_next_actions = torch.cat([agents[agent]._get_noisy_target_action(batch.shared_next_states).T for agent in env.possible_agents]).T #shape: (batch_size, sum(action_dims))
            next_state_values = torch.min(*self.target_value_model(batch.shared_next_states, shared_next_actions)) #use minimum of twin network outputs
            target_values = rewards + (self.gamma * next_state_values * (1 - is_terminals))

        q_sa_1, q_sa_2 = self.online_value_model(batch.shared_states, batch.shared_actions) #get predicted q from model for each state, action pair
        ...
        value_loss = self.value_loss_fn(torch.cat([q_sa_1, q_sa_2]), torch.cat([target_values, target_values]))
        ...
        if update_policy:
            ...
            batch.shared_actions[:, self._action_index:self._action_index+self._nA] = actions
            max_a_q_s = self.online_value_model.Q1(batch.shared_states, batch.shared_actions) #get value using online value network
...
class MADDPG():
    ...
    def _store_experience(self, state, action, reward, next_state, terminated):
        ...
        state = torch.Tensor(np.concatenate([state[agent] for agent in env.possible_agents]))
        action = torch.Tensor(np.concatenate([action[agent] for agent in env.possible_agents]))
        next_state = torch.Tensor(np.concatenate([next_state[agent] for agent in env.possible_agents]))
        reward = torch.Tensor(np.array([reward[agent] for agent in env.possible_agents]))
        terminated = torch.Tensor(np.array([terminated[agent] for agent in env.possible_agents] + [False])) #add extra value to terminals list to block potential np 2D-array conversion
        self.memory.add(Experience(state, action, reward, next_state, terminated)) #store experience in replay memory
    ...
    def train(self, env, gamma=1.0, num_episodes=100, batch_size=None, n_warmup_batches = 5, tau=0.005, target_update_steps=2, policy_update_steps=2, save_models=None, seeds=[], evaluate=True):
        ...
        batch_size = batch_size if batch_size else self.memory._batch_size
...
if __name__ == '__main__':
    ...
    replay_buffer = ReplayBuffer(storage=LazyTensorStorage(max_size=1e6, device=torch.device("cuda"))))

    seeds = np.random.randint(np.iinfo(np.int32).max, size=20, dtype=int)
    episode_returns, best_model, saved_models = maddpg.train(env, gamma=0.95, num_episodes=1000, tau=0.01, batch_size=1024, seeds=seeds,save_models=[1, 50, 100, 250, 500, 1000])
    results = {'episode_returns': episode_returns, 'best_model': best_model, 'saved_models': saved_models, 'seeds': list(seeds)}
    import pickle
    with open('testfiles/maddpg_speakerlistener_seeded1_torchrl.results', 'wb') as file:
