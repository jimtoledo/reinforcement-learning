...
#Fully-connected value network (state observation, action -> value)
...
class NormalNoiseDecayStrategy():
    def __init__(self, bounds, init_noise_ratio=0.5, min_noise_ratio=0.1, decay_steps=10000):
        self.t = 0
        self.low, self.high = bounds #action bounds ([action_mins], [action_maxs])
...
class DDPG():
    def __init__(self, 
                ...
                value_model_fn = lambda nS, nA: FCQV(nS, nA), #state vars, action vars -> model
                ...
                exploration_strategy_fn = lambda bounds: NormalNoiseDecayStrategy(bounds), #module with select_action function (model, state) -> action
    ...
    def _optimize_model(self, batch_size=None):
        ...
        max_a_q_sp = self.target_value_model(next_states, argmax_a_q_sp).detach() #get values of next states using target value network
        target_q_sa = rewards + (self.gamma * max_a_q_sp * (1 - is_terminals)) #calculate q target

        q_sa = self.online_value_model(states, actions) #get predicted q from model for each state, action pair

        #get value loss - weigh sample losses by importance sampling for bias correction
        value_loss = self.value_loss_fn(weights*q_sa, weights*target_q_sa) #calculate loss between prediction and target
        ...
        max_a_q_s = self.online_value_model(states, argmax_a_q_s) #get value using online value network
    ...
    def get_action(self, state, explore=False):
        return self.exploration_strategy.select_action(self.online_policy_model, state) if explore else self.online_policy_model.select_action(state)
    ...
    def train(self, env, gamma=1.0, num_episodes=100, batch_size=None, n_warmup_batches = 5, tau=0.005, target_update_steps=1, save_models=None, seed=None, evaluate=True):
        self.exploration_strategy = self.exploration_strategy_fn((env.action_space.low, env.action_space.high))
        ...
        for episode in tqdm(range(num_episodes)):
            ...
            for t in count():
                ...
                if len(self.memory) >= batch_size*n_warmup_batches: #optimize policy model
                    self._optimize_model(batch_size)
