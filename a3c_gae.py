...
class A3CWorker(mp.Process):
    ...
    def _optimize_model(self, rewards, values, log_probs, entropies):
        ...
        values_array = values.view(-1).data.numpy()
        ...
        gaes = np.array([np.sum(discounts[:T-1-t]*advantages[t:]) for t in range(T-1)])
        ...
        value_loss = (returns-values).pow(2).mul(0.5).mean() #mean square error
